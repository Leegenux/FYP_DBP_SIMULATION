function [H_ue, H_in] = gen_quadriga_channel_file(B, nbr_of_RB, nbr_of_ue, nbr_of_in, fname, new_generate) % check if result same
    % Example:
    ...
    if do_save && isfile(full_path)
        ...
        layout.no_rx = nbr_of_ue + nbr_of_in; % 2 receivers
        ue_ant = 'xpol';
        ...
        H = zeros(nbr_tx_ant, layout.no_rx, subc_num);

        for rx = 1:layout.no_rx
            ch = ch_merged(1, rx).fr(subc_freq * subc_num, subc_num);
            % convert to `H` dimension: #tx_antenna, #ue, #subcarrier
            H(:, rx, :) = permute(ch(1, :, :, 2), [2, 1, 3, 4]); % using 1st antenna of UE & 2nd sample
        end

        % H's normalization
        for u = 1:layout.no_rx
            norm_fac = sqrt(mean(abs(H(:, u, :)).^2, 'all'));
            H(:, u, :) = H(:, u, :) / norm_fac;
        end

        % convert to right order `horizon, vertical, polar`
        H = reshape(permute(reshape(H, nbr_bs_pol, nbr_vertical, nbr_horizon, layout.no_rx, subc_num), [3, 2, 1, 4, 5]), nbr_tx_ant, layout.no_rx, subc_num);

        H_ue = H(:, 1:nbr_of_ue, :);
        H_in = H(:, nbr_of_ue + 1:layout.no_rx, :);
