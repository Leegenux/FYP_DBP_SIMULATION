...
function [plot_param] = plot_result_emse(par, res, plot_param)
    ...
    for d = 1:res.maxiter
        ...
        if d + plot_param.next_marker - 1 == 1 % set up the attributes and things
            ...
            semilogy(par.SNRdB_list, res.EMSE(d, :), marker_style{marker_ind}, 'LineWidth', 2, 'MarkerSize', marker_size)

            grid on % enable grid
            xlabel('Es/N0 (dB)', 'FontSize', 12)
            ylabel('EMSE', 'FontSize', 12)
